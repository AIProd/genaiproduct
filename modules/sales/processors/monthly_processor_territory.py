...
class MonthlyProcessorTerritory(TimeSeriesProcessor):

    def process(self) -> Optional[pl.LazyFrame]:
        lazy_frame = self.processing_lazy_frame.sort('timestamp')

        lazy_frame = lazy_frame.with_columns(
            pl.col(constants.COLUMN_SALES).shift(12, fill_value=0).over(constants.TERRITORY_GROUP_COLUMNS).alias(constants.COLUMN_LAST_YEAR_SALES),
            pl.col(constants.COLUMN_UNITS).shift(12, fill_value=0).over(constants.TERRITORY_GROUP_COLUMNS).alias(constants.COLUMN_LAST_YEAR_UNITS),
        )
