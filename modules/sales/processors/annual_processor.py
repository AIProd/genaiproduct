...
class AnnualProcessor(TimeSeriesProcessor):
    ...
    def process(self) -> Optional[pl.LazyFrame]:
        ...
        lazy_frame = lazy_frame.with_columns(
            pl.col(constants.COLUMN_SALES).shift(1, fill_value=0).over(constants.COMMON_GROUP_COLUMNS).alias(constants.COLUMN_LAST_YEAR_SALES),
            pl.col(constants.COLUMN_UNITS).shift(1, fill_value=0).over(constants.COMMON_GROUP_COLUMNS).alias(constants.COLUMN_LAST_YEAR_UNITS),
        )
