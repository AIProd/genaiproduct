...
class HighPerformingAccounts(Finding):
    def generate(self, data: pd.DataFrame) -> Optional[List[FindingResult]]:
        ...
        mat_growth_df = data[
            data['indicator'] == constants.INDICATOR_MOVING_ANNUAL_TOTAL_CHANGE_PREVIOUS_YEAR_SALES
        ].copy()

        rolq_growth_df = data[
            data['indicator'] == constants.INDICATOR_ROLLING_QUARTER_CHANGE_PREVIOUS_YEAR_SALES
        ].copy()
        ...
        for _, row in recent_mat_growth.iterrows():
            ...
            if mat_growth_value > 0 and rolq_growth_value > 0:
                findings.append(FindingResult(
                    ...
                    type=constants.FINDING_TYPE_HIGH_PERFORMING_ACCOUNT,
                    details=f"Highperforming in {product_name} with MAT growth {mat_growth_value:.2f}% and ROLQ growth {rolq_growth_value:.2f}%",
